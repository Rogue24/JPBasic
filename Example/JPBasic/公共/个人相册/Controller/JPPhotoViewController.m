...
//            [self dismissViewControllerAnimated:YES completion:nil];
            ...
            for (JPPhotoViewModel *photoVM in self.selectedPhotoVMs) {
                [assets addObject:photoVM.asset];
            }
            
            JPGIFPreviewViewController *vc = [[JPGIFPreviewViewController alloc] init];
            vc.assets = assets;
...
- (void)setupCategoryTitleView {
    ...
    titleView.selectedIndexPathDidChange = ^(NSIndexPath *selectedIndexPath) {
        @jp_strongify(self);
        if (!self) return;
        NSInteger diffIndex = labs(self.pageCtr.selectIndex - selectedIndexPath.item);
