...
- (void)tick:(CADisplayLink *)link {
    ...
    [text addAttributes:[self textColorAttributes:color] range:NSMakeRange(0, text.length - 3)];
    [text addAttributes:[self textColorAttributes:_fpsColor] range:NSMakeRange(text.length - 3, 3)];
    self.attributedText = text;
}

- (NSDictionary *)textColorAttributes:(UIColor *)textColor {
    return @{(id)kCTForegroundColorAttributeName: (id)textColor.CGColor,
